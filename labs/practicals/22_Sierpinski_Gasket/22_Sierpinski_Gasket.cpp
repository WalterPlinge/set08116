...
void divide_triangle ( const vector<vec3> &points, unsigned int count, vector<vec3> &positions, vector<vec4> &colours )
{
	...
	if ( count > 0 )
	{
		...
		vector<vec3> new_points = { ( points[0] + points[1] ) / 2.f, ( points[0] + points[2] ) / 2.f, ( points[1] + points[2] ) / 2.f };
		// Divide new triangles
		divide_triangle ( { points[0], new_points[0], new_points[1] }, count - 1, positions, colours );
		divide_triangle ( { points[1], new_points[2], new_points[0] }, count - 1, positions, colours );
		divide_triangle ( { points[2], new_points[1], new_points[2] }, count - 1, positions, colours );
	}
	else triangle ( points, positions, colours );
	// *********************************
}
