...
{
	// The target object
	static auto& target_mesh = meshes["chaser"];

	// The ratio of pixels to rotation - remember the fov
	static const auto sh = static_cast<float>( renderer::get_screen_height ( ) );
	static const auto sw = static_cast<float>( renderer::get_screen_height ( ) );
	static const auto ratio_width = quarter_pi<double> ( ) / sw;
	static const auto ratio_height = quarter_pi<double> ( ) * ( sh / sw ) / sh;
	...
	{
		cam.set_target ( cam.get_target ( ) + normalize ( cam.get_up ( ) ) * 5.f * delta_time );
	}
	...
	{
		cam.set_target ( cam.get_target ( ) + normalize ( cross ( -cam_forward, cam.get_up ( ) ) ) * 5.f * delta_time );
	}
	...
	{
		cam.set_distance ( cam.get_distance ( ) + 5.f * delta_time );
	}
